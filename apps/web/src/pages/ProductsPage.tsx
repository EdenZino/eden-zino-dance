...
export function ProductResultPage(){
  const [params]=useSearchParams();const code=params.get('order')||'';const access=params.get('access')||'';
  const q=useQuery({queryKey:['order-result',code],queryFn:()=>api<{order:any}>(`/public/orders/${encodeURIComponent(code)}/status?access=${encodeURIComponent(access)}`),enabled:!!code,refetchInterval:(x)=>(x.state.data as any)?.order?.status==='PAID'?false:2000});
