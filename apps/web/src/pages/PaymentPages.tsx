...
export function PaymentMockPage() {
  ...
  const paymentId=params.get('payment')||'';const type=params.get('type')||'registration';const reference=params.get('reference')||'';const access=params.get('access')||'';
  const registration=useQuery({queryKey:['registration',reference],queryFn:()=>api<{registration:any}>(`/public/registrations/${encodeURIComponent(reference)}/status?access=${encodeURIComponent(access)}`),enabled:type==='registration'&&!!reference});
  const order=useQuery({queryKey:['order',reference],queryFn:()=>api<{order:any}>(`/public/orders/${encodeURIComponent(reference)}/status?access=${encodeURIComponent(access)}`),enabled:type==='order'&&!!reference});
...
export function PaymentResultPage() {
  const [params]=useSearchParams(); const code=params.get('registration')||''; const result=params.get('result'); const access=params.get('access')||'';
  const query=useQuery({queryKey:['registration-result',code],queryFn:()=>api<{registration:any}>(`/public/registrations/${encodeURIComponent(code)}/status?access=${encodeURIComponent(access)}`),enabled:!!code,refetchInterval:(q)=>['PAID','DEPOSIT_PAID','CHECKED_IN'].includes((q.state.data as any)?.registration?.status)?false:2000});
  const balance=useMutation({mutationFn:()=>api<{session:{url:string}}>('/public/payments/start',{method:'POST',body:JSON.stringify({registrationCode:code,paymentKind:'BALANCE',accessToken:access})}),onSuccess:r=>location.href=r.session.url});
